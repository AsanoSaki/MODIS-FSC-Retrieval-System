...
def predict(model, loss_function, device):
    val_p = './IMGValidation'
    out_p = './IMGPred'
    ...
    for i in range(0, len(val_path_list)):
        ...
        res = [val_path_list[i], img_shape, val_label.sum(), label_hat.sum().cpu().numpy(), MSE_val, RMSE_val, MAE_val]
...
def get_pretrained_model(model_name, device):
    ...
    if model_name == 'MLP':
        model = MLP()
        model.to(device)
        model.load_state_dict(torch.load('./save/DL_no_feature_select.params'))
...
def app():
    ...
    with st.container():
        ...
        pretrained_model, loss_function = get_pretrained_model(pretrained_model_name, device)
        pretrained_model.eval()

        st.subheader(f'{pretrained_model_name}')
        st.write('##')

        if st.button(f'Use pretrained {pretrained_model_name} predict'):
            st.write('##')

            df_pred, MSE_pred, RMSE_pred, MAE_pred = predict(pretrained_model, loss_function, device)
    ...
    # with st.container():
    #     st.write('---')
    #     st.header('Train DL Model')
    #     st.write('##')
    #
    #     st.sidebar.write('---')
    #     st.sidebar.subheader('Train DL Model')
